...
const VersionDetails: React.FC<{ name?: string }> = ({ name = "" }) => {
  ...
  const synonymFiles = React.useMemo(() => {
    const files = versionData?.files?.["species.synonym"] || [];
    ...
    return files.map((fileDetail) => {
      ...
      const fileName =
        fileDetail.filePath.split("/").pop() || "species.synonym";
      ...
      <div className="bg-slate-50/50 rounded-3xl border border-slate-100 p-8 space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 text-xs mt-1 font-medium">
              Gene synonym files for {mainSpecies}.
            </p>
          </div>
          <Button
            variant="outline"

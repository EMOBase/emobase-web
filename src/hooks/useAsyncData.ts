 ...
 * @param options.shouldRequest Additional condition to determine whether to make a
 * request, if returns `false`, the request will not be made.
 * @param options.skipFirstRender Whether to run fn on first render
 * if `true`, skip first render.
 *
 * @example
 * ```tsx
...
>(
  ...
  options?: {
    shouldRequest?: ((...deps: D) => boolean) | undefined;
    skipFirstRender?: boolean;
  },
): AsyncDataState<T, E> {
  const { shouldRequest, skipFirstRender = false } = options ?? {};

  const isFirstMount = useFirstMountState();

  const [state, setState] = useState({
    loading: skipFirstRender ? false : true,
  ...
  useEffect(() => {
    if (shouldRequest && !shouldRequest(...deps)) return;
    if (skipFirstRender && isFirstMount) return;
